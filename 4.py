...
class Conv3x31:
    ...
    def forward(self, input):
        ...
        # input_im: matrix of image
        x, h, w = input.shape
        output = np.zeros((self.num_filters * x, h - 2, w - 2))
        for im_region, i, j in self.iterate_regions(input):
            for k in range(x):
                for l in range(self.num_filters):
                    output[k * self.num_filters + l, i, j] = np.sum(im_region[k] * self.filters[l], axis=(0, 1))
        return output
    ...
    def backprop(self, d_L_d_out, learn_rate):
        ...
        for im_region, i, j in self.iterate_regions(self.last_input):
            for f in range(self.num_filters):
                ...
                # im_region: 3x3 matrix in image
                for k in range(x):
                    d_L_d_filters[f] += d_L_d_out[k * self.num_filters + f, i, j] * im_region[k]
        ...
        for i in range(d_L_d_out.shape[1]):
            for j in range(d_L_d_out.shape[2]):
                # 计算当前位置与所有滤波器的卷积结果
                for l in range(x):
                    d_L_d_input_region = np.zeros((3, 3))
                    for f in range(self.num_filters):
                        d_L_d_input_region += filters_transposed[f] * d_L_d_out[l * self.num_filters + f, i, j]
                    d_L_d_input[l, i:i + 3, j:j + 3] += d_L_d_input_region
...
conv = Conv3x3(1)  # 28x28x1 -> 26x26x1
conv1 = Conv3x31(4)  # 26x26x1 -> 24x24x4
pool1 = MaxPool2()  # 24x24x4 -> 12x12x4
conv2 = Conv3x31(2)  # 12x12x4 -> 10x10x8
pool2 = MaxPool2()  # 10x10x8 -> 5x5x8
hidden = Hidden(5 * 5 * 8, 20)  # 5x5x8 -> 20
...
def train(im, label, lr=0.01):
    ...
    gradient = hidden.backprop(gradient, lr)
...
#         pickle.dump(conv, f)
#
#     with open('pool.t.pkl', 'wb') as f:
#         pickle.dump(pool, f)
#
#     with open('hidden.t.pkl', 'wb') as f:
